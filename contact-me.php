...
Plugin URI: 
Description: Displays a contact form, for your users to e-mail you through.
Version: 1.5
Author: Podz
Author URI: 
*/
...
add_action('admin_menu', 'contactme_add_pages');

// action function for above hook
function contactme_add_pages() {
    add_options_page('Contact Me', 'Contact Me', 'administrator', 'contactme', 'contactme_options_page');
}

// contactme_options_page() displays the page content for the Test Options submenu
function contactme_options_page() {
    ...
    $opt_name_5 = 'mt_contactme_plugin_support';
    ...
    $hidden_field_name = 'mt_contactme_submit_hidden';
    ...
    $data_field_name_5 = 'mt_contactme_plugin_support';
...
<?php
    ...
    $change3 = get_option("mt_contactme_plugin_support");
...
<p><?php _e("Your E-Mail Address:", 'mt_trans_domain' ); ?> 
...
<p><?php _e("Enable Akismet Spam Checking? (Akismet plugin <strong>must</strong> be active!)", 'mt_trans_domain' ); ?> 
...
<p><?php _e("I want to support the author! (free)", 'mt_trans_domain' ); ?> 
...
function show_form() {
  ...
  $plugin_support2 = get_option("mt_contactme_plugin_support");
...
$GLOBALS["akismet_key"]		= get_option('wordpress_api_key');
$GLOBALS["akismet_home"]	= $blog_url_akismet;
$GLOBALS["akismet_ua"]		= "contactme/1.0";
...
if ($option_recaptcha=="Yes" || $option_captcha=="Yes" || $option_recaptcha=="") {
...
$emailmessage .= "Address: $sub_address1, $sub_address3, $sub_address4, $sub_address5, $sub_address6\n\n";
}
...
if ($option_address=="Yes") {
echo '<p>State/Region: <input type="text" name="address4" /></p>';
}

if ($option_address=="Yes") {
echo '<p>Zip Code: <input type="text" name="address5" /></p>';
}
...
if ($option_recaptcha=="Yes" || $option_captcha=="Yes" || $option_recaptcha=="") { echo '<p>CAPTCHA*: <center>' . recaptcha_get_html($publickey, $error) . '</center></p>'; }
...
echo "<p style='font-size:x-small'></p>";
}
...
function init_contactme_widget() {
register_sidebar_widget('Contact Me - Widget', 'show_form');
}

add_action("plugins_loaded", "init_contactme_widget");
